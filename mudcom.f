...
c     using full weighting for all real*8 2d codes
c
...
c
cccc   # !$OMP PARALLEL DO PRIVATE(i,j,ic,jc), SHARED(resf,rhsc,ncx,ncy)
	do jc=2,ncy-1
	  j = jc+jc-1
	  do ic=2,ncx-1
	    i = ic+ic-1
...
c
c     set phif,rhsf input in arrays which include
c     virtual boundaries for phi (for all 2-d real*8 codes)
c
      implicit none
...
c
cccc   # !$OMP PARALLEL DO PRIVATE(i,j,k,ic,jc,kc,rm,rk,rp)
cccc   # !$OMP+SHARED(resf,rhsc,ncx,ncy,ncz)
      do kc=2,ncz-1
	k = kc+kc-1
	do jc=2,ncy-1
	  j = jc+jc-1
...
c
cccc   # !$OMP PARALLEL DO PRIVATE(i,j,k,ic,jc,kc,rm,rk,rp)
cccc   # !$OMP+SHARED(ix,jy,kz,resf,rhsc,ncx,ncy,ncz)
      do kc=2,ncz-1
	k = kc+kz*(kc-1)
	do jc=2,ncy-1
	  j = jc+jy*(jc-1)
...
c    (y,z) interior
c
cccc   # !$OMP PARALLEL DO PRIVATE(j,k,jc,kc,rm,rk,rp)
cccc   # !$OMP+SHARED(kz,jy,ic,im1,i,ip1,resf,rhsc,ncy,ncz)
	do kc=2,ncz-1
	  k = kc+kz*(kc-1)
	  do jc=2,ncy-1
	    j = jc+jy*(jc-1)
...
c     (x,z) interior
c
cccc   # !$OMP PARALLEL DO PRIVATE(i,k,ic,kc,rm,rk,rp)
cccc   # !$OMP+SHARED(ix,kz,jc,jm1,j,jp1,resf,rhsc,ncx,ncz)
	do kc=2,ncz-1
	  k = kc+kz*(kc-1)
	  do ic=2,ncx-1
	    i = ic+ix*(ic-1)
...
c     (x,y) interior
c
cccc   # !$OMP PARALLEL DO PRIVATE(i,j,ic,jc,rm,rk,rp)
cccc   # !$OMP+SHARED(ix,jy,kc,km1,k,kp1,resf,rhsc,ncx,ncz)
	do jc=2,ncy-1
	  j = jc+jy*(jc-1)
	  do ic=2,ncx-1
	    i = ic+ix*(ic-1)
...
c     subroutines called by any real*8 mudpack solver which uses line
...
c
c     transpose n by n real*8 matrix
c
      implicit none
      integer n,i,j

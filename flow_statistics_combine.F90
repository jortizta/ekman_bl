...
! CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
! C *** Subroutine to write a combined .PLN file of the instantaneous data
! CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC

SUBROUTINE plane_XZ_combined_binary(file_name,ind)
...
!    NI = min(NXP,NXP_L)
    ...
    IF (RANK.EQ.0) THEN
      ...
      DO I=0,NX-1
	xpoint_TOT(I,:)=I*dx
	zpoint_TOT(I,:) = xpoint(1:NZ,1)
      ENDDO

    U1X_TOT(0:NXP,:)=U1X(:,1:NZ,J) 
    ...
    DO I=1,NP-1
      ...
      allocate(U1X_TEMP(NI,NZ))

      CALL MPI_RECV(U1X_TEMP(1,1),NI*NZ,8,I,0,MPI_COMM_WORLD,STATUS,IERROR)

    IF(NXP .LT. NXP_L) THEN
         U1X_TOT(I*(NXP+1):(I+1)*(NXP+1)-1,:)=U1X(:,1:NZ,J)
        ELSEIF(NXP_L .GE. 0) THEN
         U1X_TOT(I*(NXP+1):NX-1,:)=U1X(:,1:NZ,J)
    ENDIF
    ...
    CALL MPI_SEND(U1X_TEMP(1,1),NI*NZ,8,0,0,MPI_COMM_WORLD,STATUS,IERROR)
    ...
    write(*,*) 'MAX,MIN',MAXVAL(U1X_TOT),MINVAL(U1X_TOT)
    WRITE(*,*) 'NI,NZ,J,NXP,NXP_L',NI,NZ,J,NXP,NXP_L

    WRITE(6,*) 'Saving XZ-plane data on ', file_name
    OPEN(22,file=file_name,form='unformatted',status='unknown')
    WRITE(22) TIME,dx(1),xpoint(0:NZ+1,J)
    WRITE(22) NI,NZ,J,NXP,NXP_L
    WRITE(22) U1X_TOT(0:NX-1,0:NZ+1)
